...
where
	...
	T: AsRef<[u8]>,
{
    let state: Vec<Vec<FiniteField>> = state.as_ref().iter()
        .map(AsRef::as_ref)
    ...
    for c in 0..num_cols {
        result[0][c] = (state[0][c] * 0x02.into()) + (state[1][c] * 0x03.into()) + state[2][c] + state[3][c];
        result[1][c] = state[0][c] + (state[1][c] * 0x02.into()) + (state[2][c] * 0x03.into()) + state[3][c];
        result[2][c] = state[0][c] + state[1][c] + (state[2][c] * 0x02.into()) + (state[3][c] * 0x03.into());
        result[3][c] = (state[0][c] * 0x03.into()) + state[1][c] + state[2][c] + (state[3][c] * 0x02.into());
    }

    result.into_iter().map(
        |row| row.into_iter().map(Into::into).collect()
    ).collect()
}
...
where
	...
	T: AsRef<[u8]>,
{
    let state: Vec<Vec<u8>> = state.as_ref().iter()
        .map(AsRef::as_ref)
